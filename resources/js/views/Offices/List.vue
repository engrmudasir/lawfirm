...
const titleStack = ref(['Admin', 'Offices'])
...
const checked = (isChecked, office) => {
  if (isChecked) {
    checkedRows.value.push(office)
  } else {
    checkedRows.value = remove(checkedRows.value, row => row.id === office.id)
  }
}
...
<template>
  ...
  <table>
    ...
    <tbody>
      <tr
        v-for="(office, index) in itemsPaginated"
        :key="office.id"
        :class="[tableTrStyle, index % 2 === 0 ? tableTrOddStyle : '']"
      >
        <checkbox-cell
          v-if="checkable"
          @checked="checked($event, office)"
        />
        <td class="image-cell">
          <user-avatar
        ...
        <td data-label="Created">
          <small
            ...
            :title="dateFormat(office.created_at)"
          >{{ dateFormat(office.created_at) }}</small>
        </td>
        <td class="actions-cell">
          ...
          >
            <jb-button
              ...
              :icon="mdiLeadPencil"
              small
              :to="{ name: 'Update Office', params: { id: office.id } }"
            />
            <jb-button
              color="info"
              :icon="mdiEye"
